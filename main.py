...
AVAILABLE_MODELS = ["MobileNetV2", "ResNet18", "ResNet34", "ResNet50", "ResNet101", "ResNet152"]  # Add or remove model names as needed.
# Step 2: Modify the train function.
@cli.command()
@click.option('--dataset', default='cifar10', help='Dataset to train on. Either cifar10 or cifar100')
@click.option('--epochs', default=1, help='Number of epochs to train for')
@click.option('--batch_size', default=64, help='Batch size')
@click.option('--list_models', is_flag=True, callback=list_models, expose_value=False, is_eager=True, help='List available models')
@click.option('--model', default='MobileNetV2', help='Model to train. E.g., MobileNetV2')
...
@click.option('--dataset', default='cifar10', help='Dataset to test on. Either cifar10 or cifar100')
def test(model, dataset):
    """Test the model"""
    print("Loading data...")
    print("Dataset: ", dataset)
    print("Model Name: ", model)
    model_instance = tf.keras.models.load_model(f'model_output/output_{model}_{dataset}')
